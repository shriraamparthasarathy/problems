...
//algo: O(n) time complexity
//Since inorder traversal of BST is always a sorted array, the problem can be reduced to a problem
...
//point where current node value is smaller than previous node value, we update the first
//with the previous node & middle with the current node. When we find the second point
//where current node value is smaller than previous node value, we update the last with the
//current node. In case #2, we will never find the second point. So, last pointer will not
//be updated. After processing, if the last node value is null, then two swapped nodes of
//BST are adjacent.